...
const ProgramParticipants = () => {
  ...
  // Fetch participants for the program
  ...
  const handleRemoveParticipant = async (participantId) => {
    const result = await Swal.fire({
      ...
      text: 'This will remove the participant from this program.',
    ...
    if (result.isConfirmed) {
      try {
        ...
        await axios.delete(`http://localhost:8000/api/programs/${programId}/deleteparticipants`, {
          data: { participantId }
        });
        ...
        Swal.fire('Removed!', 'Participant has been removed from the program.', 'success');
      } catch (error) {
  ...
  const downloadCSV = () => {
    ...
    a.href = url;
    a.download = `program_participants_${program?.title || program?.name || 'program'}.csv`;
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-800">Program Participants</h1>
              {program && (
                <p className="text-gray-600">{program.title || program.name}</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border p-6 mb-6">
            <div className="grid md:grid-cols-2 gap-6">
              <div className="flex items-center space-x-4">
                ...
                />
                <div>
                  <h3 className="text-lg font-semibold text-gray-800">{program.title || program.name}</h3>
                  <p className="text-gray-600 text-sm">{program.description}</p>
                  <p className="text-sm text-blue-600">{program.category}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="flex items-center space-x-2">
                  ...
                  <span>
                    {new Date(program.startingDate || program.startDate).toLocaleDateString()} - 
                    {program.endingDate || program.endDate ? new Date(program.endingDate || program.endDate).toLocaleDateString() : 'Ongoing'}
                  </span>
                </div>
                <div className="flex items-center space-x-2">
                  <FontAwesomeIcon icon={faClock} className="text-gray-500" />
                  <span>{program.schedule || 'Schedule varies'}</span>
                </div>
                <div className="flex items-center space-x-2">
        ...
        <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
          {participantsLoading ? (
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-gray-500 mb-4">No participants have enrolled in this program yet</p>
            </div>
          ) : (
            <>
                    ...
                    {participantsToDisplay.map((participant) => (
                      <tr key={participant._id || participant.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <button
                            onClick={() => handleRemoveParticipant(participant._id || participant.id)}
                            disabled={deletingId === (participant._id || participant.id)}
                            className="text-red-600 hover:text-red-900 transition-colors disabled:opacity-50"
                            title="Remove from program"
                          >
                            {deletingId === (participant._id || participant.id) ? (
